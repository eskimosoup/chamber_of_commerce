module Optimadmin
  class EventAgendasController < Optimadmin::ApplicationController
    ...
    def index
      @event_agendas = Optimadmin::BaseCollectionPresenter.new(collection: EventAgenda.where("event_id = ? and name like ?", @event.id, "#{params[:search]}%").order(start_time: :desc).page(params[:page]).per(params[:per_page] || 15).order(params[:order]), view_template: view_context, presenter: Optimadmin::EventAgendaPresenter)
    end
