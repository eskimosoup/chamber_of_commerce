module Optimadmin
  class MembersController < Optimadmin::ApplicationController
    ...
    def index
      @members = Optimadmin::BaseCollectionPresenter.new(collection: Member.includes(:member_login).where('company_name ILIKE ?', "%#{params[:search]}%").page(params[:page]).per(params[:per_page] || 15).order(company_name: :asc), view_template: view_context, presenter: Optimadmin::MemberPresenter)
    end
