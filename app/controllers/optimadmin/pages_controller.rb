module Optimadmin
  class PagesController < Optimadmin::ApplicationController
    ...
    def index
      @pages = Optimadmin::BaseCollectionPresenter.new(collection: Page.page(params[:page]).per(params[:per_page] || 15).order(params[:order]), view_template: view_context, presenter: Optimadmin::PagePresenter)
    end
