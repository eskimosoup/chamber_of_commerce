module Optimadmin
  class InternalPromotionsController < Optimadmin::ApplicationController
    ...
    def index
      @internal_promotions = InternalPromotion.where('name ILIKE ?', "%#{params[:search]}%").order(params.fetch(:order, { area: :asc })).map{|x| Optimadmin::InternalPromotionPresenter.new(object: x, view_template: view_context) }
    end
  ...
  private
    ...
    def internal_promotion_params
      params.require(:internal_promotion).permit(:name, :image, :image_cache, :remove_image, :remote_image_url, :link, :area, :display, :text)
    end
  end
end
