module Optimadmin
  class InternalPromotionsController < Optimadmin::ApplicationController
    ...
    def index
      @internal_promotions = InternalPromotion.where('name ILIKE ?', "#{params[:search]}%").order(area: :asc).map{|x| Optimadmin::InternalPromotionPresenter.new(object: x, view_template: view_context) }
    end
