module Optimadmin
  class ArticlesController < Optimadmin::ApplicationController
    ...
    def index
      @articles = Optimadmin::BaseCollectionPresenter.new(collection: Article.page(params[:page]).per(params[:per_page] || 15).order(params[:order]), view_template: view_context, presenter: Optimadmin::ArticlePresenter)
    end
