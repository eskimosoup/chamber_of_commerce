module Optimadmin
  class EventCategoriesController < Optimadmin::ApplicationController
    ...
    def index
      @event_categories = Optimadmin::BaseCollectionPresenter.new(collection: EventCategory.roots.where('name LIKE ?', "#{params[:search]}%").order(position: :asc).page(params[:page]).per(params[:per_page] || 15).order(params[:order]), view_template: view_context, presenter: Optimadmin::EventCategoryPresenter)
    end
