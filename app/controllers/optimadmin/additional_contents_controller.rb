module Optimadmin
  class AdditionalContentsController < Optimadmin::ApplicationController
    ...
    def index
      @additional_contents = Optimadmin::BaseCollectionPresenter.new(collection: AdditionalContent.page(params[:page]).per(params[:per_page] || 15).order(params[:order]), view_template: view_context, presenter: Optimadmin::AdditionalContentPresenter)
    end
