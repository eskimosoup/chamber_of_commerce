module Optimadmin
  class EventLocationsController < Optimadmin::ApplicationController
    ...
    def index
      @event_locations = Optimadmin::BaseCollectionPresenter.new(collection: EventLocation.page(params[:page]).per(params[:per_page] || 15).order(params[:order]), view_template: view_context, presenter: Optimadmin::EventLocationPresenter)
    end
