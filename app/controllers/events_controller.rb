class EventsController < ApplicationController
  ...
  def index
    ...
    @presented_events = BaseCollectionPresenter.new(collection:
                        Event.upcoming.filter(params.slice(:event_location_id, :event_categories_id, :bookable, :has_tables, :food_event)).page(params[:page]).per(params[:per_page] || 15), view_template: view_context, presenter: EventPresenter
                        )
  end
